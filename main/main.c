...
void app_main(void)
{
    ...
    esp_netif_set_ip4_addr(&ip_info.ip, 10, 109, 49, 105);      // Replace with an appropriate IP address for your network
    esp_netif_set_ip4_addr(&ip_info.gw, 10, 109, 49, 1);        // Replace with your gateway IP
    esp_netif_set_ip4_addr(&ip_info.netmask, 255, 255, 255, 0); // Replace with your subnet mask
    ESP_ERROR_CHECK(esp_netif_dhcpc_stop(iface));
    ESP_ERROR_CHECK(esp_netif_set_ip_info(iface, &ip_info));
